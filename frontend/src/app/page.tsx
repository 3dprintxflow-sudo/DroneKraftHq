...
export default function Home() {
  ...
  const [missionCount, setMissionCount] = useState<string>("500+");
  ...
  const stats = [
    { icon: TrendingUp, value: missionCount,  label: "Missions Flown"   },
    { icon: LucideIcons.Users,      value: "200+",  label: "Pilots Certified" },
    { icon: Award,      value: "98%",   label: "Client Rating"    },
    { icon: Zap,        value: "24hrs", label: "Response Time"    },
  ];

  const displayServices = services.length > 0 ? services.slice(0, 4).map(s => {
    ...
    return {
      ...
      title: s.name,
      desc: s.description,
      tag: s.base_price ? `From ₹${s.base_price.toLocaleString()}` : "Contact Us"
    };
  }) : FALLBACK_SERVICES;

  return (
    <main style={{ background: "#030810", minHeight: "100vh", paddingBottom: "6rem" }}>
      ...
      <section id="services" style={{ padding: "7rem 1.5rem", maxWidth: "80rem", margin: "0 auto" }}>
        <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-60px" }}>
          ...
          <div className="services-grid">
            {displayServices.map(({ icon: Icon, title, desc, tag }) => (
              <motion.div key={title} variants={fadeUp} className="service-card">
                ...
                <div>
                  ...
                  <p className="card-desc" style={{ minHeight: "3.5rem" }}>{desc}</p>
                </div>
                <Link href="/services" className="card-cta" style={{ textDecoration: "none" }}>
          ...
          <motion.div variants={fadeUp} style={{ marginTop: "4rem", textAlign: "center" }}>
            <Link href="/services" className="btn-neon" style={{ padding: "0.875rem 2.5rem", borderRadius: "9999px", textDecoration: "none", fontSize: "0.7rem", fontWeight: 900, letterSpacing: "0.2em", display: "inline-flex" }}>
      ...
      <a
        ...
        style={{
          position: "fixed", bottom: "6.5rem", right: "1.5rem", zIndex: 100,
